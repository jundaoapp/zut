...
export function getStackList() {
  ...
  } else for (let i = 0; i < useZutContext().stackframes.length; i++) {
...
function createFrame(stackframe: StackFrame, id: number) {
  ...
  frame.addEventListener("click", () => {
    ...
    const event = new CustomEvent("zut:change-current-frame", {detail: {index: id}});
  ...
  if (stackframe.getSource().includes("?")) {
    ...
    frameFilenameBdi.append(splitFilename[0], filenameExtensionSpan, `:${stackframe.getLineNumber()}:${stackframe.getColumnNumber()}`);
  } else frameFilenameBdi.innerText = `${stackframe.getSource()}:${stackframe.getLineNumber()}:${stackframe.getColumnNumber()}`;
