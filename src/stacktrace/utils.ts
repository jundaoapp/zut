...
export async function getRawSourceMap(source: string, sourcePath: string): Promise<RawSourceMap | undefined> {
  try {
    let sourceMapURL = getSourceMapURL(source);

    if (!sourceMapURL.startsWith("data:")) sourceMapURL = getFileFolder(sourcePath) + sourceMapURL;
...
export function getSourceMapURL(source: string) {
  const sourcemapRegex = /\/\/[#@] ?sourceMappingURL=([^\s'"]+)\s*$/mg;
...
export function getFileName(sourcePath: string) {
  return sourcePath.split("/").pop();
}

export function getFileFolder(sourcePath: string) {
  return sourcePath.substring(0, sourcePath.length - getFileName(sourcePath).length);
}

export function getFileExtension(sourcePath: string) {
