...
async function enhanceFrame(frame: StackFrame): Promise<StackFrame> {
	...
	const consumer = rawSourceMap
		? await new SourceMapConsumer(rawSourceMap)
		: undefined;

	const mappedPosition = consumer
	...
	const result = {
		...
		getSourceContent(): string {
			...
			return (
				consumer?.sourceContentFor(
					mappedPosition?.source || getFileName(frame.getFileName()) || "",
				) || source
			);
		},
		...
		getFunctionName(): string {
			return (
				findFunctionName(this.getSourceContent(), this.getLineNumber()) ||
